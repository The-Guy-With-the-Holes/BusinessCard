...
function appendAboutLists(t, idx) {
  if (!About.lists.includes(t)) return;
  let targ = Get.Page.About;
...
function CreateRecommendationpage() {
  ...
  for (key in Recommendations.Types) {
    let RZ = Recommendations.Types[key];
...
function SwapRecType() {
  let t = document.getElementById('SelectRecType');
...
function CreateContactpage(t) {
  ...
  for (el in About.Contact.Methods) {
    let p = document.createElement('p'); let span = document.createElement('span');
    let a = document.createElement('a');
    let img = document.createElement('img');
    setAttributes(img, { 'class': 'ContactBtns' + ' CtBtnsx' + (el) });
    a.setAttribute('href', contactBtns[el] + ':' + About.Contact.Address[el]);
    a.append(img);
    span.append((document.createTextNode(About.Contact.Methods[el] + ': ')),a); 
    span.setAttribute('class', 'ContactKeys')
